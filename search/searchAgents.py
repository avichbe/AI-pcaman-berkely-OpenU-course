...
def cornersHeuristic(state, problem):
    ...
    while corners:
        ...
        pointsByDistance = [(corner, abs(corner[0] - currentPoint[0]) + abs(corner[1] - currentPoint[1])) for corner in
                            corners]
        #sort the points by distance
        pointsByDistance.sort(key=lambda x: x[1])
        closestPoint = pointsByDistance[0]
        currentPoint = closestPoint[0]
        corners.remove(closestPoint[0])
